...
def bookingHistory(request):
    ...
    if request.method=='GET':
        if user.is_authenticated and request.session['access_type']=='traveller':
            bookings = Order.objects.filter(customer=user,status=True,agent_approval=True).order_by('-id')
            context = {
                'Bookings':bookings,
...
def historyInvoiceGenerator(request,orderID):
    user = request.user
    if user.is_authenticated and request.session['access_type']=='traveller':
        if Payment.objects.filter(Order__order_id=orderID,Order__customer=user,Order__agent_approval=True).exists():
            invoice = Payment.objects.get(Order__order_id=orderID,Order__customer=user)
            bill_context = {
                "transactionId": invoice.transaction_id,
                "bankTransactionId": invoice.banktransaction_id,
            ...
            pdf = render_to_pdf('invoice/invoice_history.html', bill_context)
            if pdf:
                response = HttpResponse(pdf, content_type='application/pdf')
                filename = "{}.pdf".format(invoice.Order.order_id)
                content = "inline; filename={}".format(filename)
